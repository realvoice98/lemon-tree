        ...
        (err, results) => {
            if (err) {
                ...
                if (password === user.passwd) {
                    ...
                    // 비밀번호 불일치
                    res.send("<script>alert('비밀번호 불일치.');location.href='http://localhost:8001/admin';</script>");
                }
            } else {
                // 사용자 없음             
                res.send("<script>alert('사용자 없음.');location.href='http://localhost:8001/admin';</script>");
            }
        }
    );
});
